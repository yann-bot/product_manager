...
export type Sale = {
  ...
  // Valorisation COGS (module costing). `cogs` : snapshot figé à la vente
  // (null tant que non valorisée). `cogsRecalculated` : COGS d'audit après
  // rejeu du journal complet (null tant qu'aucun recalcul). `shortfallQuantity`
  // : quantité à découvert au moment de la vente (0 = couverte).
  cogs: number | null;
...
export interface StockLedger {
  ...
  reverseSale(input: {
    ...
    quantity: number;
    saleId: string;
    /** Coût unitaire de la restitution (= cogs/quantité de la vente annulée). */
    unitCost?: number;
  }): Promise<void>;
}

// Port vers la valorisation (implémenté par le module Costing). Après la
// sortie de stock, on fige le COGS de la vente par rejeu FIFO des lots.
// Défini ici pour garder le core Sales indépendant du module Costing
// (même patron que StockLedger).
export interface CostingLedger {
  costSale(input: { saleId: string; productId: string }): Promise<void>;
}

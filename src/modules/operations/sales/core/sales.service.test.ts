...
import type {
  ...
  ProductRepository,
  UpdateProductDTO,
} from "../../../catalog/product/core/product.entities";
...
class FakeStockLedger implements StockLedger {
  ...
  async reverseSale(input: LedgerCall & { unitCost?: number }): Promise<void> {
    this.reversals.push({
...
// Port de valorisation : ici un no-op (le calcul FIFO est testé séparément
// dans `modules/costing/core/fifo.test.ts`). On enregistre juste les appels.
class FakeCostingLedger implements CostingLedger {
  calls: { saleId: string; productId: string }[] = [];
...
test("create : fige le prix, calcule le total (RM-03/04/05) et sort le stock", async () => {
  ...
  const service = new SalesService(sales, new FakeProductRepository([product]), stock, new FakeCostingLedger());
...
test("createFromEasySell : repli sur le prix catalogue quand le montant EasySell est absent", async () => {
  ...
  const sales = new FakeSalesRepository();
  const service = new SalesService(sales, new FakeProductRepository([product]), new FakeStockLedger(), new FakeCostingLedger());
